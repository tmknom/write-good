...
LABEL org.label-schema.vendor="tmknom" \
      ...
      org.label-schema.description="Naive linter for English prose." \
...
ARG MODULE_VERSION=0.13.1

RUN set -x && \
    apk add --no-cache nodejs=${NODEJS_VERSION} nodejs-npm=${NODEJS_VERSION} && \
    npm install -g write-good@${MODULE_VERSION} && \
    npm cache clean --force && \
    apk del nodejs-npm

WORKDIR /work
ENTRYPOINT ["/usr/bin/write-good"]
CMD ["--help"]
